...
def compute_read_sets(C, ntrans, nullable):
    def FP(x): return dr_relation(C, x, nullable)

    def R(x): return reads_relation(C, x, nullable)
...
def yacc(
    ...
    if (optimize and lr_read_tables(tabmodule, 1)):
        ...
        if requires:
            ...
            for r, v in requires.items():
                try:
                    ...
                    print(
                        "Invalid specification for rule '%s' in require. Expected a list of strings" %
                        r)
        ...
        symbols.sort(
            lambda x,
            y: cmp(
                get_func_code(x).co_firstlineno,
                get_func_code(y).co_firstlineno))
